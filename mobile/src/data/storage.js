...
function defaultStats() {
  return { box: 0, seen: 0, correct: 0, incorrect: 0, lastSeen: null, streak: 0 };
}

function buildInitialWords() {
  const all = [...IRREGULAR_VERBS, ...COMMON_WORDS];
  const words = {};
  for (const w of all) words[w.id] = { ...w, custom: false, stats: defaultStats() };
  return words;
}
...
export async function loadState() {
  ...
  try {
    ...
    for (const w of builtinAll) {
      if (!parsed.words[w.id]) parsed.words[w.id] = { ...w, custom: false, stats: defaultStats() };
    }
    return parsed;
  } catch {
    const state = { words: buildInitialWords(), sessions: [], createdAt: Date.now() };
...
export function addCustomWord(state, en, fr, category = "custom") {
  ...
  const words = { ...state.words, [id]: { id, en, fr, category, custom: true, stats: defaultStats() } };
  return { ...state, words };
}
